...
function enableButtons () {
	...
	btn = document.getElementById("wish_for_name");
	btn.onclick=function(){wishForName();};
	// Update the button label now that the button is active
	btn.value="Click me for a personal wish for a day";
}
...
function wishForName () {
	var name = document.getElementById("name_field").value;
	var period = document.getElementById("period_field").value;
	var request = gapi.client.helloworldendpoints.wishForName({'name': name, 'period':period});
	request.execute(sayHelloCallback);
	}
